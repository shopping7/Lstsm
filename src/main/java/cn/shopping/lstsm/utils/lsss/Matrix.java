package cn.shopping.lstsm.utils.lsss;
...
public class Matrix {
    ...
    public Vector mul2(Vector a){
        if(this.cols != a.getLen() || a.isFlag())
            return null;
        Vector res = new Vector(this.rows, false,"Element");
        Element temp;
        for(int i = 0; i < this.rows; i++){
            temp = AlgorithmServiceImpl.Zr.newElement().setToZero();
            for(int j = 0; j < this.cols; j++)
                temp =temp.add(a.getValue2(j).mul(this.matrix[i][j]));
            res.setValue(i, temp);
        }
        return res;
    }
